...
class ExportHargaPanganJob implements ShouldQueue
{
    ...
    public function handle(ExportService $exportService): void
    {
        ...
        $data = HargaPangan::with(['komoditas', 'pasar'])
            ->whereIn('id', $this->ids)
            ->get();
        ...
        if ($this->type == 'pdf') {
            $exporter = new PdfExporter($json, 'export.harga-pasar-pdf', $this->pathFile);
            $exporter->export();
        } else {
            Excel::store(new ExelExporter($json, 'export.harga-pasar-exel'), $this->pathFile, 'public');
        }
