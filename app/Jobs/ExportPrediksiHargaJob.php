...
class ExportPrediksiHargaJob implements ShouldQueue
{
    ...
    public function handle(ExportService $exportService): void
    {
        ...
        $data = PrediksiHarga::with(['komoditas', 'pasar'])
            ->whereIn('id', $this->ids)
            ->get();
        ...
        $json = [
            ...
            'data' => $data,
            'start' => $data->min('tanggal_prediksi'),
            'end' => $data->max('tanggal_prediksi')
        ];

        if ($this->type == 'pdf') {
            $exporter = new PdfExporter($json, 'export.prediksi-harga-pangan-pdf', $this->pathFile);
            $exporter->export();
        } else {
            Excel::store(new ExelExporter($json, 'export.prediksi-harga-pangan-exel'), $this->pathFile, 'public');
        }
